            ...
            test('responds with json data, scraped from external website', (done) => {
                ...
                request(app)
                    ...
                    .then((res) => {
                        ...
                        expect(Object.keys(res.body.list[0]).sort()).toEqual(
                            expectedKeys
                        )
                        mockFuturesList.forEach(({ url }) =>
            ...
            test('Internal server error: Pagination number not found', (done) => {
                ...
                request(app)
                    ...
                    .then((res) => {
                        expect(res.body.name).toEqual('Internal server error')
                        expect(res.body.message).toEqual(
                            'Scraper Error: Pagination number not found'
                        )
                        done()
                    })
                    .catch((err) => done(err))
    ...
    describe('/api/future/{symbol}', () => {
        ...
        describe('#SUCCESS', () => {
            test('responds with json data, filtered by symbol', (done) => {
                ...
                mockFutureEDList.forEach(({ template }) =>
                    axios.get.mockResolvedValueOnce({ data: template })
                )
            ...
            test('responds with saved json data', (done) => {
                fs.existsSync.mockReturnValueOnce(true)
                fs.readFileSync.mockResolvedValueOnce(
                    JSON.stringify(mockJsonFutureED)
                )
